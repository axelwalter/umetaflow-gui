...
try:
    ...
    # with st.sidebar:
    #     if "file_chooser" in st.session_state:
    #         st.session_state.file_chooser = st.multiselect("mzML files", [path.name for path in Path(mzML_dir).iterdir()], st.session_state.file_chooser)
    # #     file_chooser = st.multiselect("mzML files", [path.name for path in Path(mzML_dir).iterdir()], [])


    # if st.button("run"):
    #     st.write(file_chooser)
            ...
            with st.spinner("Uploading files..."):
                for file in Path(upload_dir).iterdir():
                    if file.name not in Path(mzML_dir).iterdir() and file.name.endswith("mzML"):
                        ...
                        st.session_state.file_chooser.append(file.name)
