...
if any(Path(results_dir).glob("*.ftr")):
    ...
    for sample in df_auc.columns:
        ...
        fig.add_trace(go.Scattergl(name=sample[:-5], x=df["time"], y=df[[col for col in df if metabolite in col][0]]))
    fig.update_layout(
        title=metabolite,
        xaxis_title=f"time ("+params["time_unit"]+")",
        yaxis_title="intensity (counts per second)",
...
# except:
#     st.warning("Something went wrong. Please check your input table and use unique metabolite names!")