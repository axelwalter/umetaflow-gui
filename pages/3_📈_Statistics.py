...
and add at least one more custom attribute column to where your samples differentiate (e.g. ATTRIBUTE_Treatment: antibiotic and control).
    ...
    if not df.empty:
        ...
        c1, c2 = st.columns(2)
        c1.markdown("### Clustering and Heatmap")
        c2.markdown(" ")
        if c2.button("**🔥 Generate Heatmap**"):
            df_heat = df[[col for col in df.columns if col.endswith(".mzML")]].set_index(df["metabolite"]).T
            # df_heat = StandardScaler().fit_transform(df_heat)
            fig = ff.create_dendrogram(df_heat, labels=list(df_heat.index))
            fig.update_layout(template='plotly_white')
