...
def run_workflow(params, result_dir):
    ...
    with st.status("Loading mzML files and getting number of spectra...", expanded=True) as status:
        ...
        for file in params["example-workflow-selected-mzML-files"]:
            ...
            path = str(Path(st.session_state["workspace"], "mzML-files", file+".mzML"))
    ...
    df = pd.DataFrame({"filenames": params["example-workflow-selected-mzML-files"], "number of spectra": num_spectra})
