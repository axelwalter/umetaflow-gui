...
class Plot:
    def extracted_chroms(
        self, df_chrom, chroms=[], df_auc=None, title="", time_unit="seconds"
    ):
        colors = cycle(COLORS)
        traces_line = []
        traces_bar = []
        for chrom in chroms:
            ...
            if len(df_auc) == 1 and chrom in df_auc.columns:
                traces_bar.append(
                    go.Bar(
                        x=[chrom], y=[df_auc[chrom][0]], name=chrom, marker_color=color
                    )
                )
        fig_chrom = go.Figure()
        fig_auc = go.Figure()
        ...
        for name in trace_names:
            ...
            while name + "_" + str(i) + "_RT" in df_chrom.columns:
                ...
                if len(df_auc) == 1 and name in df_auc.columns and i == 1:
                    traces_bar.append(
                        go.Bar(
                            x=[name], y=[df_auc[name][0]], name=name, marker_color=color
                        )
                    )
                i += 1
        if len(df_auc) == 1:
            ...
            if not df_std.empty:
                fig.add_trace(
                    go.Bar(
                        ...
                        name=feature,
                        error_y=dict(
                            type="data", array=df_std.loc[feature], visible=True
                        ),
                    )
                )
            else:
    ...
    def plot_bpc(df, ms1_rt, ms2_rt=0):
        intensity = np.array(
            [max(intensity_array) for intensity_array in df["intarray"]]
        )
    ...
    def plot_peak_map_2D(df, cutoff):
        ...
        mzs = np.concatenate([df.loc[index, "mzarray"] for index in df.index])[
            int_filter
        ]
        rts = np.concatenate(
            [
...
"""
    ...
    def plot_consensus_feature_chromatograms(
        feature_maps: dict, to_plot: dict, title: str
    ):
