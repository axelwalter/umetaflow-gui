...
class Visualization:
        ...
        if name:
            ...
            if view == "2D peak map":
                ...
                if not df_ms2.empty:
                    ...
                    c1.plotly_chart(Plot.plot_ms_spectrum(df_ms1[df_ms1["RT"].round(2) == ms1_rt], f"MS1 spectrum @RT {ms1_rt}", "#EF553B"))
                    c2.plotly_chart(Plot.plot_ms_spectrum(df_ms2[df_ms2["precursormz"].round(4) == ms2_precmz], f"MS2 spectrum @precursor m/z {ms2_precmz} @RT {round(ms2_rt, 2)}", "#00CC96"))
    ...
    def display_FFM_data(feature_maps, spectra):
        ...
        if name:
            ...
            fig = Plot.plot_feature_chromatogram(df[df["metabolite"] == selected_feature])
            c2.plotly_chart(fig)
