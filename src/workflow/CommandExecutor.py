...
class CommandExecutor:
    ...
    def run_command(self, command: list[str], write_log: bool = True) -> None:
        self.logger.log("Running command:\n"+" ".join(command)+"\nWaiting for command to finish...")
        start_time = time.time()
        ...
        if write_log:
            ...
            if stderr:
                self.logger.log(f"{prefix}ERRORS OCCURED:\n\n{stderr.decode()}")
            if process.returncode == 1:
                self.logger.log(f"Process failed: {' '.join(command)}")
    ...
    def run_topp(self, tool: str, input_output: dict, show_log: bool = True) -> None:
        ...
        if  len(set(io_lengths)) > 1:
            self.logger.log(f"ERROR in {tool} input/output. Input/Output list lengths must be 1 and/or the same.")
            return
